...
function setTaskResponse(response, task) {
  if(!task.response) {
    task.response     = [];
  }
...
function taskUpdate(self, status, eventName) {
  ...
  return function(workerId, taskId, response) {
    ...
    .then(function(task) {
      ...
      task.status         = status;
      task                = setTaskResponse(response, task);
...
function Queue(store) {
  ...
  var workerNotify  = this.workerNotify.bind(this);
...
Queue.prototype.workerRegister  = function(taskTypes, worker) {
  ...
  var taskHandler = (function(handler, typeList) {
